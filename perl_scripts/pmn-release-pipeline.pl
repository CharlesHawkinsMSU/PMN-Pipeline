...
if (length $ptoolsapi > 0) {
	...
	if ($ptools > 0) {
		message("IMPORTANT: Another pathway-tools instance is running, please finish all existing pathway-tools jobs and re-run this pipeline.");
		exit(1);
	}
}
...
while ($pty->is_active) {
        ...
        if ($type[$sql] eq 'lisp') {
                ...
                print $fh "\n";
                message("Excuting $type[$sql] command: $cmd[$sql]...");
                system "$type[$sql] $cmd[$sql] > $ARGV[1]_perlcyc_$rownum[$sql] 2>&1 & ";
...
if (length $error > 0) {
        ...
        print STDERR "\n";
        message("IMPORTANT: Tasks done ($ARGV[0]) but you chose not to save changes to PGDB $args{s_pgdb}!");
}
