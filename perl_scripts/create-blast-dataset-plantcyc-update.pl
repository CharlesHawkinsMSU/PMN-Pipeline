...
###########################################################
  ...
  Usage: $0 <pgdb> <fasta-file-folder> <polypeptides-list> <version> <output-folder>
...
my ($DB, $fafolder, $polyfile, $version, $outdir) = @ARGV;
$fafolder =~ s!/$!!;
$outdir =~ s!/$!!;
my $source = 'UniProt';
...
my @enzs;
print STDERR "Opening File $polyfile\n";
open (FILE, $polyfile) || die "The File $polyfile Could Not Be Found.\n";
