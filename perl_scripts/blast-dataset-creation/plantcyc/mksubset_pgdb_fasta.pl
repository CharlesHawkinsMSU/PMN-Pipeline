...
my $fa = shift; #this is the combined all PGDB fasta file

### print out PGDB fasta entry if the PGDB protein exists in plantcyc. At the end we are getting a subset of the PGDB fasta file only for proteins that exist in plantcyc.
...
while (my $line = <FILE>) {
	...
	#print STDERR "ERROR: $id has no sequenece\n";
    }
}
