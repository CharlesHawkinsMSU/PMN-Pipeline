...
###modified again to include accession-1, this is the phytozome sequence id use in Lee's fasta sequence file.


my $cyc = perlcyc -> new ("PLANT");

print "Enzyme\tEnzyme_common_name\tEnzrxn_common_name\tSpecies\tGene\tGene_common_names\tUniProt_id\tAccession\n";
...
foreach my $enz (@ENZ){
    ...
    my $taxon = $cyc->get_slot_value($enz,"species");
    my $species = $cyc->get_slot_value($taxon, "common-name");
    my @uniprots;
    ...
    #print "$enz\t$enz_common_name\t$g\t$temp\n";
...
#Get rid of duplicates of commnon-names of genes
