...
use lib ('/shared/Projects/PMN/Belize_recovery/home/peifenz/perlcyc_1.2/');
use perlcyc;
...
### modified for special treatment for corncyc. print out accession-2 for old E2P2V1 genes. If no accession-2, print out accession-1 for new E2P2V2 genes.   
...
foreach my $enz (@ENZ){
    ...
    my @Genes;
    my %Gene_names;
    my $accession_2 = $cyc->get_slot_value($g, "accession-2");
    if (!$accession_2){
	$accession_2 =  $cyc->get_slot_value($g, "accession-1");
    }
...
#	    print "$enzrxn_cn\n";
    ...
    print "$enz\t$enz_common_name\t$enzrxn_cn_str\t$species\t$g\t$gene_cn_str\t$uniprot\t$accession_2\n";
}
