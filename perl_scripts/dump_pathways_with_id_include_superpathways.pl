...
my $out = "Pathway-id\tPathway-name\tReaction-id\tEC\tProtein-id\tProtein-name\tGene-id\tGene-name\n";
...
#    if (!@subpathways) {
	...
	if (!@proteins) {
	    $out .= "$p\t$pathwayName\t$r\t$rname\tunknown\tunknown\tunknown\tunknown\n";
	}
	else {
...
sub printProteinsReport {
	...
	if (!@genes) {
	    $out .= "$p\t$pathwayName\t$r\t$rname\t$protein\t$pname\tunknown\tunknown\n";
	}
	else {
...
sub printGenesReport {
	...
	$out .= "$p\t$pathwayName\t$r\t$rname\t$protein\t$pname\t$g\t$gname\n";
    }
}
