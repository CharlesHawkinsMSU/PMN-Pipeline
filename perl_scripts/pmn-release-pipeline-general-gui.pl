...
# exit GUI
# send INT signal to the terminal
message("Closing the GUI...");
print $fh "^C\n";
$pty->kill('INT', 1);
$error = wait_for_lisp($pty);
# message($error);
# select option to continue (exit GUI)
$error =~ /^ (\d+): Exit Pathway Tools version 21.5/m || $error =~ /^ (\d+): Return to Top Level \(an "abort" restart\)/m || $error =~ /^ (\d+): Exit event loop for/m;
message("Found option: $error");
if (!$1) {
	message("ERROR: no valid option detected for exiting GUI ($ARGV[0]).");
	&closeAll;
	exit(1);
}
...
while ($pty->is_active) { 
        ...
        if ($type[$sql] eq 'lisp') {
                ...
                $pty->write("$cmd[$sql]\n");        
                $error = wait_for_lisp($pty);
                #$error =~ /^Restart actions (select using :continue)/;
                #message($1);
        } else {
                # perl or sh
				print("We have a perl or sh command\n");
				print("SQL is $sql\n");
                ...
                print $fh "\n";
                message("Excuting $type[$sql] command: $cmd[$sql]...");
                my $foo = system "$type[$sql] $cmd[$sql] > $ARGV[1]_perlcyc_$rownum[$sql] 2>&1 & ";
				print("Command has been executed: $foo\n");
        ...
        my $errorflag = "Restart actions (select using :continue)";
        # if ($sql < $#cmd && length $error < 1) {
        if ($sql < $#cmd && index($error, $errorflag) == -1) {
        #if ($sql < $#cmd) {
                $sql ++;
        } else {
...
sub wait_for_lisp {
		...
		$fh->sync;
                ...
                $log .= $o if ($o && length $log >= 0);
                # message("Log after O $log");
                if ($o && $o =~ /EC\(\d+\):/) {
                        last;
                }
