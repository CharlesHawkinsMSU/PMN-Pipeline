...
def list_all_pathways(directory_of_interest, write_outfile):
	...
	for (dirpath, dirnames, filenames) in os.walk(directory_of_interest):
		if "/data" in dirpath:
			...
			with open(dirpath + "/reactions.dat", "rb") as readFile:
	...
	with open(write_outfile, 'w+') as f:
		for key, value in dictionary_of_all_pathways.items():
			f.write('%s\t%s\n' % (key, value))
		f.write("#" + directory_of_interest)		
