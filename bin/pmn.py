...
def read_pipeline_files(args):
	...
	try:
		...
		if args.o:
			# The initial list of organisms requested by the user
			org_list_input = args.o.split(',')
			# First expand any ranges, such as 2-7 expanding to 2,3,4,5,6,7
			org_list_exp_ranges = []
			for org in org_list_input:
				m = org_range_re.match(org)
				if m:
					start = int(m.group(1))
					end = m.group(2)
					if end == '': # Missing range end means end on the last organism; so if there are 10 orgs, 3- means 3,4,5,6,7,8,9,10
						end = len(index_table)
					else:
						end = int(end)
					if start > end:
						error(f'Request to operate on organisms {start} - {end} but {start} comes after {end}')
						exit(1)

					org_list_exp_ranges += list(range(start, end+1))
				else:
					org_list_exp_ranges.append(org)
			# Now we get the final organism list by resolving indexes to orgids and erroring out if a nonexistant org or an index past the end is requested
			org_list = []
			for org in org_list_exp_ranges:
				try:
					org = index_table[int(org)]
				except ValueError:
					pass
				except KeyError:
					error(f'Requested organism {org} but there are only {len(ptable)} orgs available')
					exit(1)
...
# The below pair of functions are used to deal with the "Unique ID" field in the input table file. Each PGDB except MetaCyc has a "Unique ID" which is not the same thing as its orgid; for example, AraCyc has the Unique ID of "QT". This Unique ID is used as part of the frame ID of any frames created in AraCyc, so for example an Enzymatic Reaction created in AraCyc would have a frame ID like ENZRXNQT-12345, in contrast to one created in MetaCyc which would look like ENZRXN-12345. This is done to ensure that these frames don't end up having name collisions with frames created in MetaCyc or other databases.
# Unique IDs are assigned sequentially by SRI, and they have a server that will give you the next available one, ensuring that no two PGDBs have the same one.
# A complication is that these IDs have two forms: the Unique ID and the org counter. The org counter is just an integer; the Unique ID is the org counter encoded as Base36. The Unique ID form is used in frame IDs and in the PMN input table, but PathoLogic expects to find the org counter form in the organism parameters file it takes as input, and SRI's Unique ID server gives out new IDs in org counter form. This means we need to be able to convert between them, hence these two functions.

# This function converts a Unique ID to an org counter. This Python function is a manual translation of a Perl function written by Chuan Wang
...
# This function is the reverse of counter_from_id above; it converts an org counter to a Unique ID
