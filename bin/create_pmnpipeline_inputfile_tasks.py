...
def retrieve_exp_from_dat(file_path, exp_codes, taxon_list):
    exp_dict = {}
    print("getting ref dict from %s, codes %s, taxa %s"%(file_path, exp_codes, taxon_list))
    with open(file_path, 'r', encoding='iso-8859-1') as fp:
        for u_id, attrs in read_dat_itr(fp, "UNIQUE-ID", " - "):
            unique_id = u_id.replace('UNIQUE-ID -', '').strip()
            # print(unique_id)
            ...
            for index, entries in enumerate(attributes):
                ...
                if fields[0] == 'SPECIES':
                    ...
                    print("%s is in species %s"%(unique_id, species_id))
    ...
    if task_type == 'reactions':
        ...
        if args.to_create_path is not None:
            ...
            with open(args.to_create_path, 'r') as tcp:
                for line in tcp:
                    ...
                    try:
                        ...
                        #if pgdb_name == 'aracyc':
                        taxa = ['ORG-' + taxon if taxon == '5993' else 'TAX-' + taxon for taxon in info[5].strip().split(',')]
                        ...
                        semi_auto_refine_b(ref_path, input_pgdb_folder, args.npp_path, taxa, exp_codes, output_dir)
                    except IndexError:
                        continue
        else:
            ...
            if ref_pgdb_name == input_pgdb_name:
                # Old to New
                semi_auto_refine_b(ref_path, input_path, args.npp_path, [], exp_codes, output_dir)
            else:
                # Metacyc to Plantcyc
                ...
                if reference_pgdb_version is not None:
                    ...
                    semi_auto_refine_b(ref_path, input_path, args.npp_path, plant_taxons, exp_codes, output_dir)
