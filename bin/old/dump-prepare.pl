...
#my $date = sprintf "%d%02d%02d-%02d%02d%02d", map { $$_[5]+1900, $$_[4]+1, $$_[3], $$_[2], $$_[1], $$_[0] } [localtime];
my $date = sprintf "%d%02d%02d", map { $$_[5]+1900, $$_[4]+1, $$_[3] } [localtime];
...
foreach my $s (@sp) {
    ...
    my $dcoutdir = File::Spec->rel2abs("$ARGV[2]-custom-files");
    $dcoutdir =~ s/^\/carnegie\/data/\/home\/cwang/;
    mkdir $dcoutdir;
    
    # write dump-custom-files.master foreach species
    open(DC, ">$ARGV[2]/$s/dump-custom-files.master") or die "Cannot open file $ARGV[2]/$s/dump-custom-files.master to write.\n";
    print DC "s_ptools\t$ARGV[3]\ns_script_folder\tperl_scripts\nperl\ts_script_folder/dump_compounds.pl $s > $dcoutdir/".lc($data{$s}[1])."_compounds.$date\nperl\ts_script_folder/dump_pathways_with_id_include_superpathways.pl $s > $dcoutdir/".lc($data{$s}[1])."_pathways.$date\n";
    close DC;
...
s_cm_folder	common
s_script_folder	perl_scripts
fs_pwy_del	pathways-to-delete
fs_pwy_add	pathways-to-import
...
lisp	(so 'plant)
lisp	(so 's_pgdb)
lisp	(import-frames :frames '(s_curator) :src-kb (find-org 'plant) :dst-kb (find-org 's_pgdb))
...
TEMP
    ...
    $outdir =~ s/^\/carnegie\/data/\/home\/cwang/;
    open(BS, ">$ARGV[2]/$s/blastset.master") or die "Cannot open file $ARGV[2]/$s/blastset.master to write.\n";
    print BS "s_ptools\t$ARGV[3]\ns_script_folder\tperl_scripts\nperl\ts_script_folder/create-blast-dataset-pgdb.pl $s \"$data{$s}[2]\" $s_seq_source $ARGV[7]/$data{$s}[6] $outdir\n";
    close BS;
