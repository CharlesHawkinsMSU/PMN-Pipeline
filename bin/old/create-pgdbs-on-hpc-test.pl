...
foreach my $fa (`ls $fafolder`) {
    ...
    chomp $ptools;
    close $in;
    $pty->spawn("srun -N 1 -n 1 --pty $ptools -patho $fafolder/$fa/ -disable-metadata-saving -nologfile -no-cel-overview -no-web-cel-overview");
    open(OUT, ">$fafolder/$fa/hpc-create-$fa.log") or die $!;
    while ($pty->is_active) {
        my $o = $pty->read(1);
        print OUT $o ? $o : '';
        OUT->flush;
