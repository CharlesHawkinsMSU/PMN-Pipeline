...
foreach my $fa (`ls $fafolder`) {
    ...
    $pty->spawn("srun -N 1 -n 1 --pty perl $FindBin::Bin/../pmn-release-pipeline/pmn-release-pipeline-general.pl $fafolder/$fa/fix.master $fafolder/$fa/fix.log
");
    open(OUT, ">$fafolder/$fa/hpc-fix-$fa.log") or die $!;
    while ($pty->is_active) {
        my $o = $pty->read(1);
        print OUT $o ? $o : '';
        OUT->flush;
